...
export default function DetalhesRequisicao({ req, aoVoltar, aoMudarStatus, aoAtualizarItens, aoAdicionarResponsavel, aoFinalizarSeparacao, recordesGlobais }) {
  ...
  useEffect(() => {
    ...
    // Calcula quantos itens físicos existem para buscar o recorde
    ...
    // Só roda o relógio se estiver "Em Separação" E a separação ainda NÃO TIVER sido finalizada
    if (req.status === 'Em Separação' && !req.metricasSeparacao) {
      ...
      // Se já foi finalizado, exibe o tempo travado
  ...
  const salvarProgresso = () => {
    ...
    if (todosBipados) {
      // É A LINHA DE CHEGADA! Pára o tempo e calcula a eficiência.
      ...
      if (metricasFinais.bateuRecorde) {
        ...
        setTimeout(() => setMostrarFesta(false), 5000); // Apaga a festa após 5 segundos
      ...
      // Salvamento parcial (Não para o tempo, apenas salva os dados)
  ...
  // --- OUTRAS AÇÕES DE STATUS E EDIÇÃO ---
  const confirmarAssumirTarefa = () => {
  ...
  const confirmarMudanca = () => {
    ...
    if (novoStatus === 'Saída de produtos') {
      ...
      // Validação Extra: O tempo DEVE ter sido finalizado
      if (!req.metricasSeparacao) {
  ...
  return (
    <div className="detalhes-container">
      ...
      <div className="card-info" style={{ position: 'relative' }}>
        ...
        {/* --- O CRONÔMETRO DE PRODUTIVIDADE NO TOPO DA TABELA --- */}
      ...
      {/* SÓ MOSTRA O BOTÃO SE AINDA NÃO FOI FINALIZADO O CRONÔMETRO */}
      {req.status === 'Em Separação' && !req.metricasSeparacao && (
        <button className="btn-salvar-progresso" onClick={salvarProgresso}>
          💾 Salvar Progresso Físico
        </button>
