...
export default function EdicaoRecebimento({
  ...
  // 🚀 MUDANÇA AQUI: Permite exibir se for Pendente OU Aguardando Precificação
  if ((status !== 'Pendente' && status !== 'Aguardando Precificação') || isEditing) return null;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginBottom: '20px' }} className="no-print">
      
      {/* CAIXA DE EDIÇÃO PONTILHADA (Aparece no Pendente e na Precificação) */}
      ...
      {/* CAIXA INICIAR CONFERÊNCIA (ETAPA 0) - SÓ APARECE SE FOR PENDENTE */}
      {status === 'Pendente' && (
